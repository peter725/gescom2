...
import { AppliedFilter, FilterService } from '@base/shared/filter';
import { TulsaQuery } from '@base/shared/filter/tulsa-query';
...
@Directive()
export abstract class BaseListFilterPageComponent<T = any, ID = number> implements OnInit, OnDestroy {
  ...
  abstract projId?: number | unknown;
  ...
  protected fetchData(): Observable<Page<T>> {
    return fromPromise(this.getRequestConfig()).pipe(
      ...
      switchMap(config => {
        ...
        //   : this.crudService.findAll(config);
        config.pathParams= {
          id: this.projId as number
        }
        return this.crudService.findAllFilter(this.projId as ID, config);
      })
    );
  }
