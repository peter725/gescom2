...
import { SampleTemplate } from '@libs/sdk/sample-template';
...
export class SampleContextService {
  ...
  private readonly activeTemplate = new BehaviorSubject<SampleTemplate | undefined>(undefined);
  ...
  private readonly availableTemplates = new BehaviorSubject<SampleTemplate[]>([]);
  ...
  useTemplate(value: SampleTemplate | undefined) {
  ...
  private templateLoader() {
    ...
    return this.crudService.findAll<SampleTemplate>(config).pipe(
      map(page => page.content),
      map(list => {
        //const userId = this.authContext.instant().getDetails().userId;
